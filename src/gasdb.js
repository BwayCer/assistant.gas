...
gasOrder('assistant/Gasdb', function (deps) {
    deps.juruo.set({
        _assistant_notExistSpreadsheetConfig:
            'The spreadsheet config Not set.',
        _assistant_notExistSheet:
            'The "{spreadsheet}" spreadsheet is not exist.',
    ...
    deps.Gasdb = function (dbMethodObject, joinTable) {
         ...
         *
         * @example
         * var dbSheet = new Gasdb('spreadsheetName', 'tableName');
         */
        function Gasdb(strSheetName, strTableName) {
            ...
            if (lenArgs !== 2)
                throw Error(
                    deps.juruo.get('__inconsistentExpectation', {
                        name: 'Gasdb',
                    })
                );

            var spreadsheetList = deps._config.spreadsheet;
            if (!spreadsheetList)
                throw Error(deps.juruo.get('_assistant_notExistSpreadsheetConfig'));
            ...
            if (cache.hasOwnProperty(tableKeyOfCache)) {
                ...
                if (cache.hasOwnProperty(sheetKeyOfCache))
                    ...
                    throw Error(
                        deps.juruo.get('_assistant_notExistSheet', {
                ...
                if (spreadsheetList[sheet_table].hasOwnProperty(strTableName))
                    ...
                    throw Error(
                        deps.juruo.get('_assistant_notExistSheetTable', {
                            spreadsheet: strSheetName,
                            table: strTableName,
                        })
                    );
        ...
        Gasdb.prototype.getUrl = function (rowIdx) {
            var range = rowIdx == null ? '' : '&range=' + rowIdx + ':' + rowIdx;
        ...
        function () {
             ...
             * @return {Array}
             */
            var joinTable = function join() {
                var arraySlice = Array.prototype.slice;
                var args = arraySlice.call(arguments);

                var newTableInfo = _getNewTableInfo(args);
                // 指定欄位 + key
                var column = newTableInfo.countColumn + 1;
                var keyList = newTableInfo.keyList.sort();
                var row = keyList.length + newTableInfo.countEmptyRow;

                var len_argu, idx_argu, val_argu;
                var len_concat, idx_concat, val_concat;
                var sourceTable, idxKey, isAllElem, pickElemList, pickElemListLength;
                var key, idxRow, rowList;
                var newTable = new Array(row);
                var idxEmptyKeyRow = keyList.length;
                var idxColumnStart = 1;
                ...
                for (
                    ...
                    pickElemListLength
                        = (isAllElem ? val_argu[1][0] : pickElemList).length;
            ...
            function _getNewTableInfo(listForTableJoinInfoList) {
                return listForTableJoinInfoList.reduce(function (
                    ...
                    if (tableJoinInfoList == null
                        || tableJoinInfoList.constructor !== Array
                    )
                        throw TypeError(
                    ...
                    var idxKey = tableJoinInfoList[0];
                    var table  = tableJoinInfoList[1];
                    ...
                    var keyList = accumlator.keyList;
                    ...
                    for (idx = 0, len = table.length; idx < len ; idx++) {
                        ...
                        if (val.length !== column)
                            throw RangeError(deps.juruo.get(
                                '_assistant_notEqualLengthColumnOfSheetTable'
                            ));
                        ...
                        if (!bisEmptyKey && ~tableKeyList.indexOf(key))
                            throw Error(deps.juruo.get('_assistant_tableSameKeys'));

                        tableKeyList.push(key);

                        if (bisEmptyKey) accumlator.countEmptyRow++;
            ...
            function _pickElemVal(
                ...
                var isAllElem = pickElemList === 'All';
