...
gasOrder('assistant/tgbot', function (deps) {
    deps.juruo.set({
        _assistant_notExistTgbot:
            'Telegram bot "{botName}" is not exist.',
    });
    ...
    deps.tgbot = function tgbot(
        ...
        if (botTokenList.hasOwnProperty(botName)) {
            ...
            throw Error(
                deps.juruo.get('_assistant_notExistTgbot', {
                    botName: botName,
                })
            );
        }
