...
assistant.order( 'assistant/logNeedle', function ( self ) {
    ...
    function logNeedle( bisPrint, objPreMsgTable ) {
        ...
        this.msgTable = !!objPreMsgTable
            ? Object.assignEacy( Object.create( this.defaultMsgTable ), objPreMsgTable )
            : Object.create( this.defaultMsgTable );

        // this.timer_clearCode = 0;
        this.codeInfo = [
            0, /* originCode */
            0, /* scriptCode */
            0, /* scriptCategory */
    ...
    logNeedle.prototype.defaultMsgTable
        = Object.assignEacy( new Object.emptyEnum(), defaultMsgTable );
     ...
     * 設定訊息碼。
     * <br><br>
     * Google Apps Script 沒有計時器。
     *
     * @memberof module:assistant.logNeedle~
     * @func _setCode
     * @param {logNeedle} self - 訊息代碼。
     * @param {Number} code - 訊息代碼。
     */
    function _setCode( self, numCode ) {
        var codeInfo = self.codeInfo;
        var strCode = numCode.toString();
        var len = strCode.length;

        codeInfo[ 0 ] = numCode;
        codeInfo[ 1 ] = Number( strCode.substring( 0, len - 2 ) );
        codeInfo[ 2 ] = Number( strCode[ len - 2 ] );
        codeInfo[ 3 ] = Number( strCode[ len - 1 ] );

        // self.timer_clearCode
        //     = self.timer_clearCode
        //     || setTimeout( _setCode.clearCode, 0, self );
    }

    // _setCode.clearCode = function ( self ) {
    //     var codeInfo = self.codeInfo;
    //     codeInfo[ 0 ] = codeInfo[ 1 ] = codeInfo[ 2 ] = codeInfo[ 3 ]
    //         = self.timer = 0;
    // };
